...
def calculate_fitness(chromosome, special_ids=None, return_details=False):
    if special_ids is None:
        try:
            ...
            special_ids = {'upacara': 1, 'bersih': 2, 'istirahat': 3, 'co_teaching': {1, 2}, 'pns_p3k': set()}
    ...
    if np.any(mask_senin_1): hc1_v += np.sum((chromosome[mask_senin_1, MAPEL_IDX] != upacara_id) | (chromosome[mask_senin_1, GURU_IDX] != -1))
    mask_kamis_1 = (chromosome[:, HARI_IDX] == 4) & (chromosome[:, WAKTU_IDX] == 1)
    if np.any(mask_kamis_1): hc1_v += np.sum((chromosome[mask_kamis_1, MAPEL_IDX] != bersih_id) | (chromosome[mask_kamis_1, GURU_IDX] != -1))
    mask_istirahat_5 = (chromosome[:, WAKTU_IDX] == 5)
    if np.any(mask_istirahat_5): hc1_v += np.sum((chromosome[mask_istirahat_5, MAPEL_IDX] != istirahat_id) | (chromosome[mask_istirahat_5, GURU_IDX] != -1))
    mask_istirahat_8 = (chromosome[:, WAKTU_IDX] == 8) & (chromosome[:, HARI_IDX] != 5)
    if np.any(mask_istirahat_8): hc1_v += np.sum((chromosome[mask_istirahat_8, MAPEL_IDX] != istirahat_id) | (chromosome[mask_istirahat_8, GURU_IDX] != -1))
    mask_jumat_pulang = (chromosome[:, HARI_IDX] == 5) & (chromosome[:, WAKTU_IDX] >= 7)
    if np.any(mask_jumat_pulang): hc1_v += np.sum((chromosome[mask_jumat_pulang, MAPEL_IDX] != -1) | (chromosome[mask_jumat_pulang, GURU_IDX] != -1))
    hc1_p = hc1_v * W_HARD
    p_total += hc1_p
    ...
    for slot_idx in multi_gene_slots:
        ...
        if has_non_co: hc2_v += (kth_counts[slot_idx] - 1)
        else:
            unique_mapels = np.unique(slot_mapels)
            if len(unique_mapels) > 1: hc2_v += (kth_counts[slot_idx] - 1)
    hc2_p = hc2_v * W_HARD
    p_total += hc2_p
    details['hc2'] = {'v': int(hc2_v), 'p': int(hc2_p)}
    ...
    if np.any(mask_with_guru):
        ...
        sc3_v += np.sum(np.maximum(0, counts - 7))
    if len(pns_p3k_ids) > 0:
        ...
        else: pns_counts_dict = {}
        for g_id in pns_list: sc3_v += max(0, 24 - pns_counts_dict.get(g_id, 0))
    sc3_p = sc3_v * W_SOFT
    p_total += sc3_p
    details['sc3'] = {'v': int(sc3_v), 'p': int(sc3_p)}
